...
const reasons = [
  ...
  "Premium amenities: clubhouse, pool, cottages & organic store",
    ...
    <div className="container mx-auto px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {reasons.map((r) => (
            <div key={r} className="flex items-start gap-3 bg-primary-foreground/5 rounded-lg p-4 backdrop-blur-sm">
              <CheckCircle2 className="w-5 h-5 text-accent shrink-0 mt-0.5" />
              <p className="text-primary-foreground/90 text-sm leading-relaxed">{r}</p>
            </div>
          ))}
        </div>
      </div>
